        ...
        <div class="responsive">
            ...
            <table class="table table-bordered table-hover">
                ...
                @forelse($users as $value)
                    ...
                    <td>
                      @if($value->role===0)
                      <i class="fas fa-ban"style="color:red; font-size:18px;cursor:pointer;" title="pending"></i>                      
                      @elseif($value->role===1)
                      <i class="far fa-check-circle"style="color:green; font-size:18px;cursor:pointer;"></i>                      
                      @endif
                    </td>
                    <td>
                      <?php $id= $value->id;?>
                      <a href="{{url('client-detail/')}}/{{$value->id}}" class="btn-default btn btn-sm">View Detail</button>
                    </td>
                    <td>
                      @if($value->role===0)
                      <button type="button" onClick="updateRole('<?php echo $value->id?>','<?php echo $value->role?>')"  class="btn-primary btn btn-sm">Approve</button>
                      @elseif($value->role===1)
                      <button type="button" onClick="updateRole('<?php echo $value->id?>','<?php echo $value->role?>')"  class="btn-danger btn btn-sm">
                        Block
                      </button>
                      @endif
                    </td>
